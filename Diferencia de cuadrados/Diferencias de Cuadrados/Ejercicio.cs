    ...
    */
    ...
    public static class DifferenceOfSquares
    {
        public static int CalculateSquareOfSum(int max)
        {
            ...
            int sum = Enumerable.Range(1, max).Sum(); // Calcular la suma de los primeros N números naturales
            int CalculateSquareOfSum = (int)Math.Pow(sum, 2); // Calcular el cuadrado de la suma
            return CalculateSquareOfSum;

        }
        public static int CalculateSumOfSquares(int max)
        {
            // Calcular la suma de los cuadrados de los primeros N números naturales
            int CalculateDifferenceOfSquares = Enumerable.Range(1, max).Select(x => x * x).Sum();

            return CalculateDifferenceOfSquares;
        }
